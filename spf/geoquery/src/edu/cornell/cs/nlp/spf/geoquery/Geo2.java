...
import edu.cornell.cs.nlp.spf.interact.Interacter;
/**
...
public class Geo2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			final Model<Sentence, LogicalExpression> model = Model.readModel(new File(dataDir,"model"));
			...
			final Interacter.Builder<Sentence,LogicalExpression,Sentence> interBuild = new Interacter.Builder<Sentence, LogicalExpression,Sentence>(parser);
			final Interacter<Sentence,LogicalExpression,Sentence> interactor = interBuild.build();

			for (Sentence s : talk) {
				interactor.interact(model, s);
			}
