 ...
 *            Labeled data item.
 */
public class Interacter<SAMPLE extends IDataItem<?>, MR, DI extends IDataItem<SAMPLE>>
		implements IInteract<SAMPLE, MR, DI> {

	public static final ILogger LOG = LoggerFactory
			.create(Interacter.class.getName());

	private final IParser<SAMPLE, MR> parser;

	private final IFilter<SAMPLE> skipParsingFilter;

	private Interacter(IFilter<SAMPLE> skipParsingFilter, IParser<SAMPLE, MR> parser) {
		this.skipParsingFilter = skipParsingFilter;
		this.parser = parser;
	}
	
	public void interact(IModelImmutable<SAMPLE, MR> model, final DI dataItem) {
		LOG.info("%s", dataItem);
		...
		if (bestModelParses.size() == 1) {
			...
			System.out.println(bestModelParses.get(0));
			...
			System.out.println(bestModelParses);
		} else {
			// No parses
			LOG.info("no parses");
	...
	public static class Builder<SAMPLE extends IDataItem<?>, MR, DI extends IDataItem<SAMPLE>> {
		...
		public Builder(IParser<SAMPLE, MR> parser) {
			this.parser = parser;
		}

		public Interacter<SAMPLE, MR, DI> build() {
			return new Interacter<SAMPLE, MR, DI>(skipParsingFilter, parser);
		}
